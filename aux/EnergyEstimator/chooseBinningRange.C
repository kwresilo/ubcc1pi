int GetEntriesInRange(TTree *pTree, const float minRange, const float maxRange)
{
    return pTree->GetEntries(("isGolden && truePdgCode == 211 && range > " + std::to_string(minRange) + " && range <= " + std::to_string(maxRange)).c_str());
}
...
void chooseBinning()
{
    ...
    //   - The first bin will start at minRange
    //   - The last bin will end be between maxRange and cutoffRange
    ...
    //     width in steps smaller than minDeltaRange
    const auto minRange = 5.f;
    const auto maxRange = 200.f;
    const auto cutoffRange = 300.f;
    const auto minDeltaRange = 0.01f;
    ...
    const auto minEntriesPerBinTarget = 400;
    const auto maxEntriesPerBinTarget = 3400;
    const auto entriesPerBinTargetStep = 200;
    ...
    {
        // The initial range step to use when choosing bins, this is changed dynamically to converge on the bin size quickly
        const auto maxRangeStep = (maxRange - minRange) * (static_cast<float>(nEntriesPerBinTarget) / static_cast<float>(GetEntriesInRange(particles, minRange, maxRange)));
        ...
        auto rangeStep = maxRangeStep;
        auto binLower = minRange;
        auto binUpper = binLower + rangeStep;
        auto nEntriesLast = 0;
        ...
        while (binUpperVector.empty() || binUpperVector.back() < maxRange)
        {
            const auto nEntries = GetEntriesInRange(particles, binLower, binUpper);

            // Save the details of the current bin if it has the correct number of entries, or we have passed a cutoff threshold
            //   - std::abs(rangeStep) < minDeltaRange --> avoid fine tuning bins
            //   - binUpper >= cutoffRange             --> avoid an infinite final bin
            if (nEntries == nEntriesPerBinTarget || std::abs(rangeStep) < minDeltaRange || binUpper >= cutoffRange)
            {
                // Save the details
                ...
                rangeStep = maxRangeStep;
                binLower = binUpper;
                binUpper = binLower + rangeStep;
                nEntriesLast = 0;

                // Bail if the last bin is getting huge
                if (binUpper >= cutoffRange)
                    break;
            }
            else
            {
                ...
                rangeStep = std::abs(rangeStep) * ((nEntries > nEntriesPerBinTarget) ? -1 : 1) * (((nEntries - nEntriesPerBinTarget) * (nEntriesLast - nEntriesPerBinTarget) > 0) ? 1.f : 0.5f);
            }

            // Grow (or shrink) the current bin
            binUpper = std::min(binUpper + rangeStep, cutoffRange);
        ...
        h1->GetXaxis()->SetTitle("Reco range / cm");
        h1->GetYaxis()->SetTitle("Number of reco particles");
        particles->Draw("range >> h1", "isGolden && truePdgCode == 211");
        SaveAs(c, "goldenPionRange_target" + std::to_string(nEntriesPerBinTarget) + "EntriesPerBin");
        ...
        h1Scaled->GetXaxis()->SetTitle("Reco range / cm");
        h1Scaled->GetYaxis()->SetTitle("Number of reco particles / (bin width / cm)");
        h1Scaled->Draw();
        SaveAs(c, "goldenPionRange_target" + std::to_string(nEntriesPerBinTarget) + "EntriesPerBin_scaled");
        ...
        h2->GetXaxis()->SetTitle("True range / cm");
        h2->GetYaxis()->SetTitle("Reco range / cm");
        h2->GetZaxis()->SetTitle("Number of reco particles");
        h2->GetZaxis()->SetTitleOffset(1.6);
        particles->Draw("range:trueRange >> h2", "isGolden && truePdgCode == 211", "colz");
        SaveAs(c, "goldenPionRange_recoToTrue_target" + std::to_string(nEntriesPerBinTarget) + "EntriesPerBin");
        ...
        for(unsigned int i = 1; i <= nBins; ++i)
        {
            for(unsigned int j = 1; j <= nBins; ++j)
            {
                ...
                h2Equal->SetBinContent(i, j, scaled);
            }
        }

        h2Scaled->GetXaxis()->SetTitle("True range / cm");
        h2Scaled->GetYaxis()->SetTitle("Reco range / cm");
        h2Scaled->GetZaxis()->SetTitle("Number of reco particles / (bin area / cm^2)");
        h2Scaled->GetZaxis()->SetTitleOffset(1.6);
        h2Scaled->Draw("colz");
        SaveAs(c, "goldenPionRange_recoToTrue_target" + std::to_string(nEntriesPerBinTarget) + "EntriesPerBin_scaled");
        ...
        h2Equal->GetXaxis()->SetTitle("True range / cm");
        h2Equal->GetYaxis()->SetTitle("Reco range / cm");
        h2Equal->GetZaxis()->SetTitle("Number of reco particles / (bin area / cm^2)");
        ...
        SaveAs(c, "goldenPionRange_recoToTrue_target" + std::to_string(nEntriesPerBinTarget) + "EntriesPerBin_equal");
        ...
        for(unsigned int i = 1; i <= nBins; ++i)
        {
            ...
            auto totalTrue = 0.f;
            for (unsigned int j = 1; j < nBins; ++j)
                totalTrue += h2Scaled->GetBinContent(i, j);

            for(unsigned int j = 1; j <= nBins; ++j)
            {
                // Scale down by the total true entries to get a probability
                const auto migrationValue = h2Scaled->GetBinContent(i, j) / totalTrue;
        ...
        h2Migration->GetXaxis()->SetTitle("True range / cm");
        h2Migration->GetYaxis()->SetTitle("Reco range / cm");
        ...
        SaveAs(c, "goldenPionRange_recoToTrue_target" + std::to_string(nEntriesPerBinTarget) + "EntriesPerBin_migration");
