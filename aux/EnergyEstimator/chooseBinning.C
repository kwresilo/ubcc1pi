...
void chooseBinning()
{
    ...
    const auto minDeltaMomentum = std::numeric_limits<float>::epsilon(); //0.00001f;

    // Choose the range of bins to use
    const auto minBins = 2;
    const auto maxBins = 12;
    const auto binStep = 1;

    // Setup the output vectors
    std::vector<float> nBinsVector;
    std::vector<float> diagonalityVector;

    // Do the loop over target entries per bin
    for (auto nBinsTarget = minBins; nBinsTarget <= maxBins; nBinsTarget += binStep)
    {
        const auto totalEntriesFloat = static_cast<float>(GetEntriesInMomentumRange(particles, minMomentum, maxMomentum));
        const auto nEntriesPerBinTarget = static_cast<int>(std::ceil(totalEntriesFloat / static_cast<float> (nBinsTarget)));

        // The initial momentum step to use when choosing bins, this is changed dynamically to converge on the bin size quickly
        const auto maxMomentumStep = (maxMomentum - minMomentum) / static_cast<float>(nBinsTarget);
        ...
        std::cout << "=====================================================" << std::endl;
        std::cout << "  - nBinsTarget:          " << nBinsTarget << std::endl;
        ...
        {
            diagonality = diagonalSum / (diagonalSum + offDiagonalSum);
            std::cout << "  - diagonality:          " << diagonality << std::endl;
        }

        nBinsVector.push_back(static_cast<float>(nBins));
        diagonalityVector.push_back(diagonality);
    ...
    // Make the diaonality plot
    const auto nPoints = nBinsVector.size();
    const auto &nBinsArray = nBinsVector.data();
    const auto &diagonalityArray = diagonalityVector.data();

    TGraph *g = new TGraph(nPoints, nBinsArray, diagonalityArray);
    g->Draw("AL");
    SaveAs(c, "nBinsDiagonality");
