/**
 *  @file  ubcc1pi_standalone/Macros/PlotProtonMomentumOptimization.cxx
 *
 *  @brief The implementation file of the PlotProtonMomentumOptimization macro
 */
...
void PlotProtonMomentumOptimization(const Config &config)
{
    // -------------------------------------------------------------------------------------------------------------------------------------
    ...
    for (const auto &[selectionName, enabledMap] : config.extractXSecs.crossSectionIsEnabled)
    {
        for (const auto &[xsecName, isEnabled] : enabledMap)
        {
            ...
            const auto dataScaled = getTrimmedMatrix("data_NuWroScaled");
            const auto dataUnscaled = getTrimmedMatrix("data_NuWroUnscaled");
            // const auto dataTrue = getTrimmedMatrixTrue("data"); //getTrimmedMatrixTrue("data");

            // Get the smearing matrix
            const auto smearingMatrixScaled = getMatrix("smearingMatrix_NuWroScaled");
            const auto smearingMatrixUnscaled = getMatrix("smearingMatrix_NuWroUnscaled");
            ...
            for (const std::string &quantity : {"data", "smearingMatrix"})
            {
                ...
                const auto statUncertaintiesScaled = (quantity == "data") ? getMatrixFunction("data_stat_NuWroScaled") : ubsmear::UBMatrixHelper::GetZeroMatrix(nBins, 1);
                const auto statUncertaintiesUnscaled = (quantity == "data") ? getMatrixFunction("data_stat_NuWroUnscaled") : ubsmear::UBMatrixHelper::GetZeroMatrix(nBins, 1);
                    ...
                    }))
                {
                    ...
                    for (const auto &[paramName, nUniverses] : dimensions)
                    {
                        ...
                        const auto biasVectorScaled = getMatrixFunction(quantity + "_" + group + "_" + paramName + "_bias_NuWroScaled");
                        const auto covarianceMatrixScaled = getMatrixFunction(quantity + "_" + group + "_" + paramName + "_covariance_NuWroScaled");

                        const auto biasVectorUnscaled = getMatrixFunction(quantity + "_" + group + "_" + paramName + "_bias_NuWroUnscaled");
                        const auto covarianceMatrixUnscaled = getMatrixFunction(quantity + "_" + group + "_" + paramName + "_covariance_NuWroUnscaled");
            ...
            const std::string prefix = "xsec_" + selectionName + "_" + xsecName;
            ...
            for (const auto &[quantity, groupToMatrixMap] : errorMatrixMapScaled)
            {
                ...
                PlottingHelper::PlotErrorMatrix(totalErrorMatrix, prefix + "_" + quantity + "_totalErrorMatrix_NuWroScaled", metadata);
                PlottingHelper::PlotFractionalErrorMatrix(totalErrorMatrix, quantityVector, prefix + "_" + quantity + "_totalFracErrorMatrix_NuWroScaled", metadata);

                // Plot the total error matrix for each group individually
                for (const auto &[group, errorMatrix] : groupToMatrixMap)
                {
                    PlottingHelper::PlotErrorMatrix(errorMatrix, prefix + "_" + quantity + "_" + group + "_totalErrorMatrix_NuWroScaled", metadata);
                    PlottingHelper::PlotFractionalErrorMatrix(errorMatrix, quantityVector, prefix + "_" + quantity + "_" + group + "_totalFracErrorMatrix_NuWroScaled", metadata);
                }
            }
            ...
            for (const auto &[quantity, groupToMatrixMap] : errorMatrixMapUnscaled)
            {
                ...
                PlottingHelper::PlotErrorMatrix(totalErrorMatrix, prefix + "_" + quantity + "_totalErrorMatrix_NuWroUnscaled", metadata);
                PlottingHelper::PlotFractionalErrorMatrix(totalErrorMatrix, quantityVector, prefix + "_" + quantity + "_totalFracErrorMatrix_NuWroUnscaled", metadata);

                // Plot the total error matrix for each group individually
                for (const auto &[group, errorMatrix] : groupToMatrixMap)
                {
                    PlottingHelper::PlotErrorMatrix(errorMatrix, prefix + "_" + quantity + "_" + group + "_totalErrorMatrix_NuWroUnscaled", metadata);
                    PlottingHelper::PlotFractionalErrorMatrix(errorMatrix, quantityVector, prefix + "_" + quantity + "_" + group + "_totalFracErrorMatrix_NuWroUnscaled", metadata);
                }
            }
            ...
            PlottingHelper::PlotErrorMatrix(smearingMatrixScaled, prefix + "_smearingMatrix_NuWroScaled", metadata, true, false);
            PlottingHelper::PlotErrorMatrix(smearingMatrixUnscaled, prefix + "_smearingMatrix_NuWroUnscaled", metadata, true, false);

            // Now get the predicted cross-section and it's error matrix
            const auto predictionScaled = getMatrix("prediction_NuWroScaled");
            const auto predictionBiasVectorScaled = getMatrix("prediction_stat_bias_NuWroScaled");
            const auto predictionCovarianceMatrixScaled = getMatrix("prediction_stat_covariance_NuWroScaled");
            ...
            const auto predictionUnscaled = getMatrix("prediction_NuWroUnscaled");
            const auto predictionBiasVectorUnscaled = getMatrix("prediction_stat_bias_NuWroUnscaled");
            const auto predictionCovarianceMatrixUnscaled = getMatrix("prediction_stat_covariance_NuWroUnscaled");
            ...
            PlottingHelper::PlotErrorMatrix(smearedPredictionErrorMatrixScaled, prefix + "_smearedPrediction_totalErrorMatrix_NuWroScaled", metadata);
            PlottingHelper::PlotFractionalErrorMatrix(smearedPredictionErrorMatrixScaled, smearedPredictionScaled, prefix + "_smearedPrediction_totalFracErrorMatrix_NuWroScaled", metadata);

            PlottingHelper::PlotErrorMatrix(smearedPredictionErrorMatrixUnscaled, prefix + "_smearedPrediction_totalErrorMatrix_NuWroUnscaled", metadata);
            PlottingHelper::PlotFractionalErrorMatrix(smearedPredictionErrorMatrixUnscaled, smearedPredictionUnscaled, prefix + "_smearedPrediction_totalFracErrorMatrix_NuWroUnscaled", metadata);

            // Save the forward-folded prediction
            FormattingHelper::SaveMatrix(smearedPredictionNuWro, "xsec_" + selectionName + "_" + xsecName + "_forwardFoldedPrediction_NuWro.txt");
            ...
            auto pDataHistScaled = std::make_shared<TH1F>((prefix + "_data_NuWroScaled").c_str(), "", binEdges.size() - 1, binEdges.data());
            auto pDataStatOnlyHistScaled = std::make_shared<TH1F>((prefix + "_dataStatOnly_NuWroScaled").c_str(), "", binEdges.size() - 1, binEdges.data());
            auto pPredictionHistScaled = std::make_shared<TH1F>((prefix + "_prediction_NuWroScaled").c_str(), "", binEdges.size() - 1, binEdges.data());

            auto pDataHistUnscaled = std::make_shared<TH1F>((prefix + "_data_NuWroUnscaled").c_str(), "", binEdges.size() - 1, binEdges.data());
            auto pDataStatOnlyHistUnscaled = std::make_shared<TH1F>((prefix + "_dataStatOnly_NuWroUnscaled").c_str(), "", binEdges.size() - 1, binEdges.data());
            auto pPredictionHistUnscaled = std::make_shared<TH1F>((prefix + "_prediction_NuWroUnscaled").c_str(), "", binEdges.size() - 1, binEdges.data());
            ...
            PlottingHelper::SetLineStyle(pDataHistUnscaled, PlottingHelper::Tertiary);
            PlottingHelper::SetLineStyle(pDataStatOnlyHistUnscaled, PlottingHelper::Tertiary);
            ...
            PlottingHelper::SaveCanvas(pCanvas, prefix + "_data-vs-smearedPrediction_NuWro");
        }
    }

    // Save the table
    tableScaled.WriteToFile("XSecPlots_goodnessOfFitStatistics_NuWroScaled.md");
    tableUnscaled.WriteToFile("XSecPlots_goodnessOfFitStatistics_NuWroUnscaled.md");
}
