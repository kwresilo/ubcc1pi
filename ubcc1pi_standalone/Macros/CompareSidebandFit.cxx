/**
 *  @file  ubcc1pi_standalone/Macros/CompareSidebandFit.cxx
 *
 *  @brief The implementation file of the CompareSidebandFit macro
 */
...
void CompareSidebandFit(const Config &config)
{
    ...
    // selectionName, name, multiplot
    std::map<std::string,std::map<std::string, PlottingHelper::MultiPlot>> multiplots;
    ...
    })
    {
        ...
        for (const auto &selectionName : {"generic", "golden"})
        {
            ...
            {
                multiPlots[selectionName].emplace_back(name, PlottingHelper::MultiPlot(yLabel, extendedBinEdges, true, config.global.axisTitles));
    ...
    for (const auto &selectionName : {"generic", "golden"})
    {
        ...
        {
            multiPlots[selectionName].emplace_back("total", PlottingHelper::MultiPlot(yLabel, {-1.f, 1.f}, true, config.global.axisTitles));
    ...
    for (const auto run: config.global.runs)
    {
        ...
        for (const auto &[sampleType, sampleName, fileName, normalisation] : inputData)
        {
            ...
            for (unsigned int i = 0; i < nEvents; ++i)
            {
                ...
                if (isDataBNB)
                {
                    std::cout<<"CompareSidebandFit Error - The BNB data is not supported in this version of the code"<<std::endl;
                }
